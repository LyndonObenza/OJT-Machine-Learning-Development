...
# ── Attach current trend (for flip detection) ─────────────────────────────────
results_df = results_df.merge(
    df[["datetime", "trend"]].rename(columns={"trend": "trend_current"}),
    on="datetime", how="left"
)
results_df["trend_prev"] = results_df["trend_current"].shift(1)
results_df["is_flip"]    = results_df["trend_current"] != results_df["trend_prev"]

# ── ALL BARS — accuracy per horizon ──────────────────────────────────────────
...
# ── FLIP BARS ONLY — true signal test ────────────────────────────────────────
flip_df = results_df[results_df["is_flip"]].copy()
print(f"\n── Flip bars only (PSAR flipped today) — {len(flip_df):,} bars ──────────")
print(f"{'Horizon':<8} {'Accuracy':>10} {'Pred↑%':>8} {'True↑%':>8} {'Rows':>8}")
print("─" * 50)
for h in range(1, 5):
    pred_col   = f"pred_t{h}"
    actual_col = f"actual_target_t{h}"
    sub = flip_df.dropna(subset=[actual_col])
    if len(sub) == 0:
        continue
    acc     = (sub[pred_col] == sub[actual_col].astype(int)).mean()
    pred_up = sub[pred_col].mean() * 100
...
naive_flip_acc = 0.5
print(f"\nNaive acc on flips (random): {naive_flip_acc:.4f}")
print(f"Flip bars / total          : {len(flip_df):,} / {len(results_df):,} ({len(flip_df)/len(results_df)*100:.1f}%)")
